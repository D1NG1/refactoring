    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    ...
                    if (p.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        thisAmount += rate * (p.getAudience() - c);
                    }
                    break;
                case "comedy":
            ...
            result.append(String.format("  %s: %s (%s seats)%n",
                    play.getName(),
                    frmt.format(thisAmount / formatRate),
                    p.getAudience()));
            totalAmount += thisAmount;
        }
        result.append(String.format("Amount owed is %s%n", frmt.format(totalAmount / formatRate)));
